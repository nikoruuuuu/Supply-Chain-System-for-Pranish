...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `manufacturer_list` where id = '{$_GET['id']}' and `delete_flag` = 0 ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
	<form action="" id="manufacturer-form">
		...
		<div class="form-group">
			...
			<input type="text" name="phone_number" id="phone_number" class="form-control form-control-sm rounded-0" value="<?php echo isset($phone_number) ? $phone_number : ''; ?>"  required/>
		</div>
		<div class="form-group">
...
<script>
	$(document).ready(function(){
		$('#manufacturer-form').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f=save_manufacturer",
				...
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
						...
						uni_modal("<i class='fa fa-th-list'></i> manufacturer Details ","manufacturer/view_manufacturer.php?id="+resp.cid)
