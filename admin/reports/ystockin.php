                ...
                <div class="container-fluid" id="printout">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            $g_total = 0;
                            $i = 1;
                            $stock = $conn->query("SELECT s.*, i.name as `item`, c.name as `category`, i.unit FROM `stockin_list` s inner join `item_list` i on s.item_id = i.id inner join category_list c on i.category_id = c.id where date_format(s.date, '%Y') = '{$year}' order by date(s.`date`) asc");
        ...
        <div class="col-2 text-center">
            ...
            <div class="col-8">
                <div style="line-height:1em">
                    ...
                    <div class="font-weight-bold h5 mb-0"><large>Pranish Inc. Supply Chain System Yearly Stock-In Report</large></div>
                    <div class="font-weight-bold h5 mb-0">as of <?= date("F Y", strtotime($year."-01")) ?></div>
                </div>
            </div>
        </div>
        <hr>
...
<script>
    ...
    $(function(){
        $('#filter-form').submit(function(e){
            e.preventDefault()
            location.href = './?page=reports/ystockin&'+$(this).serialize()
        })
