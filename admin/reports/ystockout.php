...
<div class="content py-5 px-3 bg-gradient-blue">
    <h2>Yearly Stock-Out Reports</h2>
</div>
                    ...
                    <form action="" id="filter-form">
                        <div class="row align-items-end">
                            ...
                            <div class="form-group">
                                    ...
                                    <input type="year" class="form-control form-control-sm rounded-0" name="year" id="year" value="<?= $year ?>" required="required">
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                                <div class="form-group">
                ...
                <div class="container-fluid" id="printout">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            $g_total = 0;
                            $i = 1;
                            $stock = $conn->query("SELECT s.*, i.name as `item`, c.name as `category`, i.unit FROM `stockout_list` s inner join `item_list` i on s.item_id = i.id inner join category_list c on i.category_id = c.id where date_format(s.date, '%Y') = '{$year}' order by date(s.`date`) asc");
...
<noscript id="print-header">
    <div>
        ...
        <div class="col-2 text-center">
                <img src="././logo.jpg"  alt="" class="rounded-circle border" style="width: 5em;height: 5em;object-fit:cover;object-position:center center">
            </div>
            <!-- <div class="col-8">
            ...
            <div class="col-8">
                <div style="line-height:1em">
                    <!-- <div class="text-center font-weight-bold h5 mb-0"><large><?= $_settings->info('name') ?></large></div> -->
                    <div class="font-weight-bold h5 mb-0"><large>Pranish Inc. Supply Chain System Yearly Stock-Out Report</large></div>
                    <div class="font-weight-bold h5 mb-0">as of <?= date("F Y", strtotime($month."-01")) ?></div>
                </div>
            </div>
        </div>
        <hr>
...
<script>
    ...
    $(function(){
        $('#filter-form').submit(function(e){
            e.preventDefault()
            location.href = './?page=reports/ystockout&'+$(this).serialize()
        })
