			...
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<?php 
					$i = 1;
						...
						- COALESCE((SELECT SUM(quantity) FROM `waste_list` where item_id = i.id),0) ) as `available` from `item_list`
						i inner join category_list c on i.category_id = c.id where i.delete_flag = 0 order by i.`id` asc ");
