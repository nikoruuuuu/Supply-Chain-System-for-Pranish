...
<div class="card card-outline rounded-0 card-blue">
	<div class="card-header">
		<h3 class="card-title">List of Packaging</h3>
        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT * from `packaging_list` where delete_flag = 0 order by `id` asc ");
...
<script>
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Are you sure to delete this Packaging permanently?","delete_packaging",[$(this).attr('data-id')])
		})
		$('#create_new').click(function(){
			uni_modal("<i class='far fa-plus-square'></i> Add New Packaging ","packaging/manage_packaging.php")
		})
		$('.edit-data').click(function(){
			uni_modal("<i class='fa fa-edit'></i> Add New Packaging ","packaging/manage_packaging.php?id="+$(this).attr('data-id'))
		})
		$('.view-data').click(function(){
			uni_modal("<i class='fa fa-th-list'></i> Packaging Details ","packaging/view_packaging.php?id="+$(this).attr('data-id'))
		})
		$('.table').dataTable({
			columnDefs: [
	...
	function delete_packaging($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=delete_packaging",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
